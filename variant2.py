...
class MultiLayerFCNet(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # Updated connected layer input size for 2x2
        self.fc = nn.Linear(5 * 5 * 256, output_size)
        ...
        # self.B5 = nn.BatchNorm2d(128)
        # self.layer6 = nn.Conv2d(128, 256, 5, padding=2, stride=1)
        # self.B6 = nn.BatchNorm2d(256)
        # # Updated connected layer input size for 5x5
        # self.fc = nn.Linear(4 * 4 * 256, output_size)

    def forward(self, x):
        x = self.B1(F.leaky_relu(self.layer1(x)))
        x = self.Maxpool(self.B2(F.leaky_relu(self.layer2(x))))
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if val_loss < best_loss:
                best_loss = val_loss
                patience_counter = 0
                torch.save(model.state_dict(), 'best_model_variant2.pth')
            else:
                patience_counter += 1
    ...
    model.load_state_dict(torch.load('best_model_variant2.pth'))
    model.eval()
    test_correct = 0
    test_total = 0
    with torch.no_grad():
